...
% nDOF_direct_solution  Calculate frequency response functions given system
% coefficients.
%
% FRF = nDOF_direct_solution(masses, stiffnesses, damping, freq, FRF_type)
% 
% Inputs:
%   masses is nDOF vector of masses stiffnesses is nDOF+1 vector of
%   stiffnesses, with the first and last entries being connections to
%   ground dampings is nDOF+1 vector of stiffnesses, with the first and
%   last entries being connections to ground freq is a nFreq vector of
%   frequencies FRF_type is one of
%   'admittance','mobility','accelerance','stiffness','impedance', or
%   'mass'
%
% Output:
%   FRF is a nFreq-by-nDOF-by-nDOF matrix containing the requested
% frequency response function between all pairs of degrees of freedom
...
for iFreq = 1:nFreq
    ...
    switch FRF_type
        case 'stiffness'
            FRF(iFreq,:,:) = 1./(A\eye(nDOF));
        case 'impedance'
            FRF(iFreq,:,:) = 1./(1j*w*A\eye(nDOF));
        case 'mass'
            FRF(iFreq,:,:) = 1./(-w^2*A\eye(nDOF));
