...
%% Calculate the sound pressure of a dipole
% The sound pressure from a dipole source is given by
% 
% $$
%   p_{\textrm{dipole}}  = -j \frac{ \rho_0 c_0 k^2 S \, u \, d}{
...
p_dipole      = 0;
p_dipole      = abs(  ...
                    (-1i *rho_density*c_speed_sound*    ...
                    k_wavenum.^2 * Q_volume_velocity * d_separation) /  ...
                    (4*pi*r_radius) * cos(theta*pi/180)         ...
                        );
...
% $$
%
SPL_dipole      = 20*log10(p_dipole/p_ref/sqrt(2));
...
% average intensity.
max_SPL = max(SPL_dipole);

SPL_dipole_norm=SPL_dipole-max_SPL;

%Alter the results where the SPL_dipole_norm < -100 to SPL=0
indexes=find(SPL_dipole_norm<-60);
SPL_dipole_norm(indexes)=-60;

%% Plot the SPL versus angle
%p1h=Dirplot(theta,SPL_dipole,'',[80 0 8]);
%title('SPL vs Angle for a Dipole Source');

p1h=polarplot( (theta*pi/180),SPL_dipole_norm,'k-');
...
% From Eq. 5.29 Bies and Hansen edition 5
Q_rms = Q_volume_velocity / sqrt(2);

W_D = (rho_density*c_speed_sound) * ...
        k_wavenum.^4 * (d_separation/2)^2 *  (Q_rms^2) /    ...
        (3*pi * (1+k_wavenum^2 * a_radius^2) );

% From Eq. 5.30 Bies and Hansen edition 5
p_squared=  3*W_D*rho_density*c_speed_sound/(4*pi*r_radius^2)*  ...
            (cos(theta*pi/180)).^2;

SPL_dipole_v2      = 10*log10(p_squared/p_ref.^2);

figure
p2h=polarplot(theta*pi/180,SPL_dipole_v2);
set(p2h(1),'linewidth',3);
title('SPL vs Angle for a Dipole Source');

hA2=gca;
fontname ='Times New Roman';
set(hA2,'FontName',fontname);
set(hA2,'FontSize',16);
set(hA2,'defaultaxesfontname',fontname);
set(hA2,'defaulttextfontname',fontname);

% Set orientation of 0 degrees. Default is on the horizontal.
% If you want 0 deg on vertical top, then change to 'top'
set(hA2,'ThetaZeroLocation','top');    

% Change the direction of the angles to clockwise is increasing.
set(hA2,'ThetaDir','clockwise');

% Set the limits for the radial axis
set(hA2,'RLim',[0 80]);    

% Set angle for Radial Axis numbers to 30 deg
set(hA2,'RAxisLocation',30);  
